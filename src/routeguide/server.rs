...
use routeguide::{ClientRouteNote, Feature, Point, Rectangle, RouteSummary, ServerRouteNote};
...
impl RouteGuide for RouteGuideService {
    ...
    type RouteChatStream =
        Pin<Box<dyn Stream<Item = Result<ServerRouteNote, Status>> + Send + 'static>>;

    async fn route_chat(
        &self,
        request: Request<tonic::Streaming<ClientRouteNote>>,
        ...
        let output = async_stream::try_stream! {
            while let Some(note) = stream.next().await {
                ...
                for note in location_notes {
                    yield ServerRouteNote {
                        message: format!("Hello {}!", note.message)
                    }
                }
            }
        };
